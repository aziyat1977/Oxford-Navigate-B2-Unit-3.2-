  ...
  return (
    <div className="h-full w-full flex flex-col items-center justify-center relative select-none">
      ...
      <AnimatePresence mode="wait">
        ...
        {phase === 'intro' && (
             <motion.div key="intro" exit={{opacity: 0, scale: 1.1}} className="text-center z-20 px-6">
        ...
        {phase === 'ritual' && (
          ...
          >
             <p className="mb-8 font-body text-2xl text-ink dark:text-parchment italic text-center max-w-md">
                 "Place your hand upon the Orb to attest your magical signature."
             </p>
              ...
              >
                 {/* The Crystal Ball */}
                 <motion.div 
                   className="w-48 h-48 rounded-full bg-gradient-to-br from-blue-200 to-indigo-600 dark:from-purple-900 dark:to-black shadow-[0_0_30px_rgba(75,0,130,0.5)] border-4 border-magic-gold/30 flex items-center justify-center relative overflow-hidden"
        ...
        {phase === 'diagnostic' && (
          <motion.div
            ...
            className={`w-full max-w-xl px-12 py-10 z-20 rounded-lg shadow-2xl border-4 border-double border-ink dark:border-magic-gold transition-colors duration-500 bg-parchment dark:bg-obsidian bg-paper-texture dark:bg-leather-texture
             ...
             <div className="flex flex-col gap-4">
                 {diagnosticQuestions[qIndex].options.map((opt, i) => (
                     <button
                        ...
                        className="group relative px-6 py-4 border-2 border-ink/20 dark:border-magic-gold/40 hover:border-magic-gold hover:bg-ink/5 dark:hover:bg-magic-gold/10 text-left transition-all"
                     >
        ...
        {phase === 'result' && (
          <motion.div
             key="result-ui"
             initial={{ opacity: 0 }} animate={{ opacity: 1 }}
             className="text-center z-20 px-4 bg-parchment dark:bg-obsidian p-10 rounded-lg border-4 border-magic-gold shadow-2xl"
          >
